...
Trivial tier / no card / non-pull_request event: skipped (exit 0).

Ported from loop repo gates/gate_verdict.py (behavior already correct).
"""
...
def main():
    ...
    if not pr_num:
        print("SKIP: cannot determine PR number (non-pull_request event)")
        sys.exit(0)
